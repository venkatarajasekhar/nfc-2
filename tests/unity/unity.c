...
// PATCH: clean initialization for embedded systems
    Unity.TestFailures = 0;
    Unity.TestIgnores = 0;
    Unity.CurrentTestFailed = 0;
    Unity.CurrentTestIgnored = 0;
    memset(&(Unity.AbortFrame), 0, sizeof Unity.AbortFrame);
}
...
{
    volatile unsigned int *consAddr = (unsigned int*)0x0800053c;
    consAddr[0] = code;

    asm("set 0x08000218, %g1");
    asm("clr [ %g1 ]");
    asm("mov %g0, %asr19");
    asm("nop");
    asm("nop");
...
//    UnityPrint(" Ignored\n");
    ...
    {
        UnityPrint("OK\n");
// PATCH: to fix UnityEnd() being called twice problem and return (0)
        #ifdef CONFIG_TARGET
            exit(0);
        #endif
    }
